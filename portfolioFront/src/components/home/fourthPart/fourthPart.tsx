        ...
        <motion.div
         ...
         viewport={{ once: true, amount: 0.8 }}  className="row">
          ...
          <Col lg={8} sm={8} xs={8} className="d-flex align-items-center">
            {isLoading ? 'loading' : homes.map((item: any) => <motion.p variants={cardVariants}  className="yrsa position-relative zindex-sticky aboutText" key={item.attributes.about}> {item.attributes.about}</motion.p>)}
            </Col>
        </motion.div>
      </Container>
      <Row className="pb-5">
        <Col lg={12} sm={12} xs={12} className=" p-0">
          ...
          {isLoading ? 'loading' : homes.map((item: any) => <div className="noSelect"  key={item.attributes.texteDefile} data-text={item.attributes.texteDefile}>
            <span className="noSelect"> {item.attributes.texteDefile} </span>
            </div>
            )}
          </div>
        </Col>
